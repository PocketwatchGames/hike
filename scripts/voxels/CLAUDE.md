...
- **Sliding-window `LightMap`.** The current map is sized to the entire world bounds — at the target scale that's >100 GB of texture. Replace with a player-centric window covering the streaming radius (re-centered as the player crosses chunk boundaries). Since light is baked into each chunk blob, populating the window is just a copy — no propagation pass on load.
  - **This also fixes a present-day perf cost, not just scale.** `LightMap.Upload` does `ImageTexture3D.Update(_imageList)`, which re-uploads the *entire* texture every flush (no partial 3D-texture update in Godot). So anything that dirties a chunk every frame — a flickering light, a moving torch — forces a full-world re-upload per frame (measured ~10 ms/frame with ~13 flickering lights). Band-aids in place: `ChunkManager` throttles the flush to `light_flush_hz` (default 30) and flicker is distance-gated to the player (`SimData.BlockLightFlickerCullDistance`). The windowed map is the real fix — a window-sized texture makes the full `Update` cheap, so the throttle/gate could be relaxed. If the window is still too big to re-upload whole, the deeper fix is a partial-upload path (per-chunk sub-textures or a `Texture2DArray` updated per-slice).
  - **Known visual artifact (intermittent, unfixed):** with frequent full re-uploads, the whole **water surface** was seen blinking dark for single frames at the upload cadence (≈ the flicker rate), outdoors. `water_clip_cap.gdshader` samples `light_map` for color (sun_mask + block_lit) but not for ALPHA, so it can't be a discard — it's the *whole* sample reading ~zero for a frame, which darkens everything reading `light_map` (terrain too, just less noticeable than the water losing its sheen). The block-light **data** is consistent (a flicker's remove+re-deposit is atomic within one tick, and `LightFlush` reads after), so this is a GPU **texture-update-vs-sample race** on `ImageTexture3D.Update`, not a data dropout. Hard to reproduce once the throttle (≤30 Hz uploads) + flicker distance-gate landed. Cheap targeted fix if it resurfaces: **double-buffer the LightMap** (write a back `ImageTexture3D`, then swap the `light_map` global) so sampling never races the write — costs 2× the (large) texture memory. The windowed/partial-upload map fixes it too and supersedes the double-buffer.
