...
- **Player-centric windowed volume maps (`WindowedVolumeMap.cs`).** The five `ImageTexture3D` mirrors (`LightMap`, `SkyExposureMap`, `FogMap`, `WindMap`, `WaterCurrentMap`) share a toroidal window covering the load radius rather than spanning the whole world. `ChunkManager` recenters them on chunk crossings; they are visual-only (gameplay reads the CPU `WorldState` arrays). See `WindowedVolumeMap.cs` for the toroidal addressing, recenter, no-seam reasoning, and the overlook-backdrop caveat, and `ChunkManager` for the `light_flush_hz` / flicker-gate band-aids that windowing now lets us relax.
