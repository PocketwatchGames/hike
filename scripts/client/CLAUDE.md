...
Give the source a flat quad proxy — a `MeshInstance3D` with a `PlaneMesh` on **layer 5** and an **unshaded, alpha-blended, `disable_fog = true`** material showing the mark texture (see `resources/materials/scorch_stain.tres`). The shader composites whatever the quad renders.

**`disable_fog` is not optional.** The projector's `SubViewport` runs `own_world_3d = false`, so it shares `SceneViewport`'s `World3D` and therefore its `Environment` — including the built-in depth fog. The projector camera sits `CAMERA_HEIGHT = 100 m` above the marks, so every proxy renders at ~100 m depth and picks up whatever fog is in force at that range. That fogged colour lands in `stain.rgb`, which `apply_ground_stain` **adds** into `base` (`+ stain.rgb * weight`) before the lighting split — so a black mark stops being a pure darken and becomes an additive, then-lit tint of the fog colour, and the terrain fragment carrying it gets fogged a second time by the main camera. Symptom: marks glow with the horizon colour instead of darkening. This hid for a long time only because the fog colour used to be pure black, where the bug is a no-op.

Shaders that set `POSITION` directly under `skip_vertex_transform` (the block-light shadow casters) never get a view-space `VERTEX`, so built-in fog can't reach them and they don't need the flag. `StandardMaterial3D` proxies go through the normal vertex pipeline and do.
