...
// BlockData picks one of these via [Export], and GroundTypeResolver maps a
// world-space position to the BlockData under the player's feet (overlay
// first, then the voxel's flat tile), so a single dictionary on Player/Mob
// keyed by this enum drives all footstep emission.
//
// Decoupled from VoxelType and from the kit set on purpose: many blocks map
