...
// equipped ArmorData names the MeshInstance3D parts it shows for the live gender
// (GetWornMeshNames); this composites those with the always-on base (head + face)
// and the per-slot
...
// (ArmorData.wornMeshNamesFemale / wornMeshNamesMale).
public partial class Player : CharacterBody3D
{
	...
	private void AppendSlotMeshes(EInventorySlot slot, string[] bareFallback, List<string> dst)
	{
		if (_inventory.GetEquipped(slot) is ArmorState armor
			...
			&& armorData.GetWornMeshNames(_gender) is { Length: > 0 } worn)
		{
			dst.AddRange(worn);
		}
		else
