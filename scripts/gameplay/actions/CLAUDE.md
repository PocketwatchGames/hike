...
- **Repeat-swing** (`ItemAction.repeatActionOverrides` : `Array<ActionRepeatOverride>`, `WeaponState.repeatIndex` / `repeatExpireMs`): author *one* tier; its `repeatActionOverrides` array length IS the combo length and index IS the swing number. `EnterActive` walks `repeatIndex` through the list per press (wrapping to 0 after the last), and each `ActionRepeatOverride` layers per-swing tweaks over the base tier: `damageMultiplier` (applied in `ResolveHit`), `cooldownSeconds`, `animName` (replaces the anim of every PlayAnim event in that swing's Active timeline; the clip still resolves through `WeaponData.animSet`), and `releaseEffect` (replaces the tier's release one-shot). Activating a tier with an empty list (a charged finisher) closes the repeat window so the next basic press restarts at swing 0. Light authoring for the common "same swing N times, finisher is beefier" case (club = 3 swings, knife = 5).
