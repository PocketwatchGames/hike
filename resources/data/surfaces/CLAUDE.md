...
**Never type an `atlasBaseIndex`** — it is read-only in the inspector. Add the surface to the manifest and hit Rebuild Atlas; that mints the next free index for any surface still at `Unassigned` (`-1`) and saves it back into the surface `.tres`. An index already assigned is never touched. `BlockCatalog.ValidateOrLog` errors if a block wears a surface still unassigned (i.e. one never added to the manifest, or added without a rebuild), and `block_check` is the quick way to see that.
