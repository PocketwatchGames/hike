...
A central, save-persisted bank of named variables that conditions/actions read and write **by name** to branch mob conversations and behaviors — quest progress (staged `int`), permanent world flags (`bool` "boss defeated"), counters. The bank (`ScriptVariableBank`) lives on `WorldSimState.ScriptVars` (reachable via `World.WorldState.SimState.ScriptVars`), is seeded from an authored `ScriptVariableRegistry` at world creation (`WorldState` ctor), and serializes through `SaveGame` (v3). Values are `Bool` (stored as `0/1`) or `Int`, both held as one `long`.

**References are raw `StringName`s** (mod-friendly, quick to author), kept safe by a declared set + two-layer validation rather than typed resource refs:
...
**Read** with `ScriptVarCondition` (conversation entry/response gate) or `ScriptVarTransition` (behavior-tree edge) — both `[Export] variable` + `EScriptVarCompareOp op` (`IsTrue`/`IsFalse`/`Equal`/`Greater…`) + `operand`. **Write** with `SetScriptVarAction` (conversation action) — `variable` + `EScriptVarSetOp` (`Set`/`Add`) + `operand`. All three are thin wrappers over `ScriptVarOps.Compare`/`Apply`. **Adding a use:** author the variable `.tres` + register it, then drop a condition/action sub-resource into the conversation/brain `.tres` referencing its name. **Adding a write source beyond conversations** (e.g. on boss death): call `world.WorldState.SimState.ScriptVars.SetBool(id, true)` from the gameplay event.
...
### Item Composition & Weapon Status-Effect Mods (`scripts/data/items/ItemDescriptor.cs`, `scripts/data/items/StatusEffectDescriptor.cs`)
...
**Adding a new weapon mod:** (1) add the payload field(s) to `WeaponModData` (`scripts/data/effects/payloads/`), not to `StatusEffectData` itself (and if you add a brand-new payload `*Data` class, mark it `[Tool]` — see Key Conventions, or the editor binds it as a base `Resource` and the field reads empty); (2) in the consuming code, read them off `WeaponState.statusEffects` via a small scope-filtered query method (mirror `ProjectilePierceCount(int)`, which dereferences `data.weaponMod`); (3) author a `duration = 0` `.tres` under `resources/data/status_effects/` with a `weaponMod` sub-resource; (4) compose it onto the item with a `StatusEffectDescriptor` (scope + chargeIndex) in the spawn/loot `.tres`. Never bake a mod into the shared `WeaponData` or onto an `ItemAction`.

**Composed weapon names** — a modded item's display name is generated, not authored: `WeaponNameGenerator.Compose` (`scripts/gameplay/items/`) wraps the item's base `displayName` with each weapon mod's name affix → "Fragile bomb of Piercing". The affix lives on `WeaponModData` (`affix` + `affixPosition` Prefix/Suffix); an empty `affix` falls back to the effect's `displayName`, so an ordinary adjective mod names the weapon with zero extra authoring, and you set `affix` explicitly only for a different surface form (a suffix `"of Lightning"` whose effect name is just "Lightning"). **Grammar is per-language, not in C#:** word order and connectors live in the `weapon_name_prefix` / `weapon_name_suffix` loc templates (`%0`/`%1`), so a language with post-nominal adjectives reorders the placeholders in its own `.tsv` rather than touching the generator. Read-side only and identification-gated — `WorldSimState.GetItemDisplayName(ItemState)` withholds affixes until the item is identified; the bare `GetItemDisplayName(ItemData)` overload still returns the plain noun for call sites that only have the data.

**A `weaponMod` is always weapon-attached — including on mobs.** Mobs are backed by a natural `WeaponState` (`Mob.Weapon`), created lazily the first time the mob is given a weapon mod. Mob attacks set it as the action's `primaryItem` (`BehaviorAttack`), so the **same** item-side weapon-mod path the player uses (`TriggerWeaponModChains`, `ProjectilePierceCount`, on-hit enchants, knockback) fires for mobs too. The mob weapon carries no `WeaponData` — mob damage still comes from `MobData.damageProfiles` (`ItemEventHandlers.ResolveHit` / `DoProjectile` fall back to it when the weapon yields no profile). **An elite is a mob-mod, not a body status effect:** at spawn (`Mob.Initialize`) a zone signature whose effect has a `weaponMod` payload (e.g. `status_elite_lightning`) is composed onto `Mob.Weapon` via `AddWeaponMod(AllAttacks)`; a signature with no `weaponMod` (a plain body buff) is still added to the mob's own controller. The chosen signature lives on `MobSimState.EliteStatusEffect` for the HUD elite icon and save/load, so persistence needs no change. Chain lightning therefore rides the weapon for player and elite alike; the actor-side `StatusEffectController.TriggerAttackImpact` fires only `attackImpact` body auras (no chain branch). Keep mob attack tiers at `cooldownSeconds = 0` (the behavior's `attackCooldownSeconds` governs cadence) — with `primaryItem` now set, the `ActionRunner` also tracks the mob weapon's per-tier cooldown.
