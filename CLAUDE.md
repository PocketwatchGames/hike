...
**Adding a new weapon mod:** (1) add the payload field(s) to `WeaponModData` (`scripts/data/effects/payloads/`), not to `StatusEffectData` itself; (2) in the consuming code, read them off `WeaponState.statusEffects` via a small scope-filtered query method (mirror `ProjectilePierceCount(int)`, which dereferences `data.weaponMod`); (3) author a `duration = 0` `.tres` under `resources/data/status_effects/` with a `weaponMod` sub-resource; (4) compose it onto the item with a `StatusEffectDescriptor` (scope + chargeIndex) in the spawn/loot `.tres`. Never bake a mod into the shared `WeaponData` or onto an `ItemAction`.
