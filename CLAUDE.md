...
The mod itself is an ordinary `StatusEffectData` authored with `duration = 0` (a permanent passive), carrying weapon-modifier **payload** fields — `projectilesDetonateOnContact`, `projectilePierceCount` (creatures a shot passes *through* before stopping; 0 = stops on the first), and future ones (see StatusEffectData's "Weapon modifiers" section). The payload says *what* the mod does; it is intrinsic to the effect. Each `ItemState` owns a `StatusEffectController` (`statusEffects`) constructed with null actor/world/health, so item-side mods are passive modifiers — no fx, no DoT — that travel with the object (a dropped enchanted sword keeps its enchant).
...
**Adding a new weapon mod:** (1) add the payload field(s) to `StatusEffectData`; (2) in the consuming code, read them off `WeaponState.statusEffects` via a small scope-filtered query method (mirror `ProjectilePierceCount(int)`); (3) author a `duration = 0` `.tres` under `resources/data/status_effects/`; (4) compose it onto the item with a `StatusEffectDescriptor` (scope + chargeIndex) in the spawn/loot `.tres`. Never bake a mod into the shared `WeaponData` or onto an `ItemAction`.
