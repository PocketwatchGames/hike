...
A cascade's drop is **air** in the voxel grid and stays air (standing water there floated the player back up it), so the fall is drawn by an entity instead. Worldgen's terrain approach measures the sheet once (`HeightMap.Waterfalls` → `WaterfallSite.Cells`, a column list including one cell of lateral dilation into the rock beside it), `WorldGen.PlaceWaterfalls` files a `WaterfallSimState` at the lip, and `WaterfallMeshBuilder` skins those cells at spawn with **the same unit-quad culling `WaterMesher` uses on real water** — the ribbon IS the water that used to stand there, which is what keeps it lined up with the pool it pours from.

`shaders/waterfall.gdshader` mirrors `voxel_water`'s composite (same `water_absorption` / `water_scatter_color` / `foam_*` globals, same lightmap + cloud-shadow + block-light + eye-adaptation path, same manual screen-texture blend with `ALPHA = 1`), so a fall reacts to light and to per-zone water colour exactly as the pool does. **The one thing it cannot share is thickness**: a pool reads its depth off the depth buffer, but a curtain hanging in open air would measure the far side of the valley, so thickness is authored per size tier and modulated by the flow streaks (thin streaks are the gaps you see through).

Size is one authored axis: `SimData.waterfalls` (a `WaterfallData`) holds three `WaterfallTierData` tiers picked by fall height, each carrying its own looping base ambience, lip/base spray `Fx` scenes, sheet thickness and foam. Spray emitters are **spaced** along the lip and landing lines rather than counted, so a one-column trickle gets one plume and a wide curtain mists along its whole width. A fall shorter than the first tier's `minFallHeight` draws nothing.
